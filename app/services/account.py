...
def add_account(accountForm: AccountForm) -> AccountModel:
    ...
    account = account_dao.insert(
        accountForm.joinType if accountForm.joinType else "user",
    ...
    if accountForm.profileImage:
        file_dao.insert(
            "accountProfileImage",
            account.accountId,
            accountForm.profileImage.realName,
            accountForm.profileImage.fileUrl,
            accountForm.profileImage.fileSize,
        )
    return account
    ...
    if profileImage:
        ...
        if not fileId or fileId == 0:
            file_dao.delete_using_flag(
                linkInfo="accountProfileImage", linkKey=accountForm.accountId
            )

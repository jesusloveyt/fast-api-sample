...
async def get_file_list(
    ...
    try:
        ...
        api_result.data = file_service.get_file_list(
            schTxt=schTxt, linkInfo=linkInfo, listCount=listCount, skipCount=skipCount
        )
...
async def get_presigned_url(
    fileSavedName: Optional[str] = Query(None), fileType: Optional[str] = Query(None)
) -> ApiResult[dict]:
    try:

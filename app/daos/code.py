...
def get_list(
    ...
    with session_maker() as session:
        ...
        if code:
            code_list_query = code_list_query.filter(CodeModel.code == code)
        if schTxt:
...
def get_by_UK(parentCode: str, code: str) -> CodeModel:
    with session_maker() as session:
        code = (
            session.query(CodeModel)
            .filter(
                CodeModel.useFlag == True,
                CodeModel.parentCode == parentCode,
                CodeModel.code == code,
            )
            .first()
        )
...
def update(
    ...
    with session_maker.begin() as session:
        ...
        if memo:
            update_values[CodeModel.memo] = memo
        if stringValue:
            update_values[CodeModel.stringValue] = stringValue
        if numberValue:
            update_values[CodeModel.numberValue] = numberValue

        session.execute(
            Update(CodeModel).where(CodeModel.codeId == codeId).values(update_values)
        )
        return True
